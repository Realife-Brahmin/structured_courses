...
One could consider using the left or right derivative at $x=0.5$, but this is nonstandard and not recommended for Newton's method. Instead, we implement a fallback: if $x_n$ gets too close to $0.5$, we switch to the fixed point iteration (FPI) method from Part 2, and can switch back to Newton once away from the singularity.
