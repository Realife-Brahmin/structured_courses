...
Using variable-step solvers, set $h_{max} = 1 \times 10^{-2}$ and both error tolerances $\varepsilon_{abs} = \varepsilon_{rel} = 10^{-3}$. Experiment with all the available variable-step solvers. In a table, print the maximum time-step size $h_{max}$ chosen by each solver and the total number of steps taken. Analyze the performance of different solvers for this stiff system (as determined in Q10) and determine which solver is most efficient and effective.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|l|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Summary of Solver Performance for Circuit B}
...
Unlike Q8 where we had analytical solutions for RMSE comparison, here we must rely on visual convergence of the solutions. Some key observations:

\begin{itemize}
    \item ode15s shows poor damping characteristics, which is more pronounced in this stiff system
