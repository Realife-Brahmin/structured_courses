...
subtitle(ax1, sprintf('$h \\in [%.3f,\\,%.3f]~\\mu\\mathrm{s}$', ...
        hEuler(1)*1e6, hEuler(end)*1e6), ...
...
subtitle(ax2, sprintf('$h \\in [%.3f,\\,%.3f]~\\mu\\mathrm{s}$', hT5(1)*1e6, hT5(end)*1e6), ...
         'Interpreter','latex','FontSize',11,'Color','k');
...
fprintf('h_max guide (\x3bcs):  Euler ≈ %.3f \x3bcs | Taylor-5 ≈ %.3f \x3bcs\n', hEmax*1e6, hTmax*1e6);
