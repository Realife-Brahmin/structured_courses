...
\begin{tabular}{|c|c|c|p{5.0cm}|}
\hline
\rowcolor{mediumGray}
\textbf{Method} & \textbf{$R_C$} & \textbf{Chatter} & \textbf{Remark} \\
\hline
\rowcolor{lightGray}
\textbf{BE} & $h/C$ & {\color{lushGreen} \textbf{None}} & $i(t) = 0$ (damped) as $t \geq 2h$ \\
\hline
\rowcolor{lightGray}
\textbf{Trapz} & $h/(2C)$ & {\color{lushGreen} $\downarrow$} & Bounded oscillations persist \\
\hline
\rowcolor{lightGray}
\textbf{FE} & $h/C$ & {\color{wineRed} Unstable} & Amplification: $(R_c/R_\epsilon)^n$ \\
\hline
\end{tabular}
