...
\begin{tcolorbox}[colback=white, colframe=black, title=Implementation Notes]
\textbf{Deviations from Provided OMP Algorithm:}

This implementation deviates from the originally provided pseudocode in two key aspects:
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Column Normalization:} When selecting the next best column to match the residual, the algorithm normalizes all candidate column vectors before computing their inner products with the residual. This ensures that column selection is based on directional alignment rather than magnitude. This normalization follows the standard OMP formulation which assumes $L_2$-normalized columns~\cite{Cai2011Jun}.
    
    \item \textbf{Fixed Iteration Count:} The number of columns to be selected is hard-constrained before execution, rather than iterating until the residual falls below a prespecified tolerance threshold. This provides deterministic behavior and clearer experimental comparison.
\end{enumerate}
\end{tcolorbox}
...
The Orthogonal Matching Pursuit algorithm was implemented following the pseudocode provided in the course materials \cite{panchenko_448_notes}. Key components include:
...
\textbf{Conclusion:} Matrix B demonstrates successful compressed sensing with $m=2$ measurements recovering a $n=3$ dimensional 1-sparse signal, confirming that $m < n$ recovery is possible when: (1) signal is sparse, (2) $s \geq$ true sparsity, and (3) normalized correlations are used. This aligns with the compressed sensing theory presented in the course \cite{panchenko_448_notes}.
